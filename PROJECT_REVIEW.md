...
Reviewed baseline: `AD2IOT-1123`, including a clean ESP-IDF 6.0/GCC 15 build, direct USB recovery, stable Ethernet link/DHCP/SNTP, trusted HTTPS/WSS, configuration/API workloads, and serial/network-CLI diagnostics on the primary OLIMEX hardware.
...
- Confirmed `version.txt` is the firmware identity source and advanced it to `AD2IOT-1123`; CMake observes the file so incremental builds cannot retain the prior identity.
...
This section records repository/build checks and the direct-USB boot validation for `AD2IOT-1123`. Full network validation remains gated on an Ethernet link.

- `tools/ci/validate_project.py`: pass.
- `python -m unittest discover -s tools/ci/tests -p "test_*.py"`: pass; fifty security-default, Web authorization/session, WSS history allocation, bounded-config, SD version-policy/order/UI, authenticated outbound TLS/time, incremental-version dependency, hardware-helper, firmware size/identity, bounded-copy, factory-reset, packaging, exact toolchain/dependency, removed-API, and RMII clock regression tests.
- `node --check contrib/webUI/flash-drive/www/app.js`: pass.
- `python -m compileall -q tools/ci tools/hil`: pass.
- `git diff --check`: pass.
- Clean `pio run -e esp32-poe-iso`: pass on PlatformIO Espressif32 7.0.0, ESP-IDF 6.0.0, GCC 15.2, CMake 3.30.2, and esptool 4.11 without project-source compiler warnings or deprecated/obsolete diagnostics. The build used 63,448 bytes static RAM (19.4%) and 1,581,957 bytes application flash (86.2%), below the 98,304/1,650,000-byte CI budgets. Relative to `AD2IOT-1122`, the recovery adds 8 bytes RAM and saves 948 bytes linked flash, leaving 68,043 bytes of the conservative application-flash budget.
- `pio run -e esp32-poe-iso -t buildfs`: pass.
- Firmware image inspection: pass; the 1,582,368-byte `firmware.bin` contains `AD2IOT-1123` and the `Aug 12 2026 09:31:01` build timestamp.

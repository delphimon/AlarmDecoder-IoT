...
Reviewed baseline: `AD2IOT-1125`, including a summary-first activity feed corrected for the fixed-width Alpha padding observed on `AD2IOT-1124` hardware. `AD2IOT-1123` remains the latest version to complete the full primary-OLIMEX hardware workload, including direct USB recovery, Ethernet link/DHCP/SNTP, trusted HTTPS/WSS, configuration/API workloads, and serial/network-CLI diagnostics.
...
AlarmDecoder-IoT is a capable ESP32 alarm-panel appliance with Ethernet/Wi-Fi, an AlarmDecoder parser, MQTT/Home Assistant support, browser controls, optional HTTPS/WSS, serial-over-TCP, serial and authenticated network CLIs, notification providers, SD/SPIFFS configuration, and SD firmware update support. The recent `AD2IOT-1108` through `AD2IOT-1125` work materially improved operator visibility, reliability, and maintainability: exact activity timestamps, a summary-first activity feed with optional raw detail, readable zone cards, firmware inspection/update and restart controls, a Settings pane, bounded diagnostic history, network-CLI log access, optional rotating SD logs, TLS resource diagnostics, authenticated Web/API access, bounded configuration streaming, strict SD release ordering, verified outbound TLS with trusted boot time, and an ESP-IDF 6.0/GCC 15 migration with exact managed-component resolution and project-owned RMII clock configuration are now present.
...
The dependency-free host regression suite has grown from six to fifty-five checks covering externally reachable service defaults, Web UI authorization guard placement, WSS session state/history allocation, activity burst consolidation, fixed-width Alpha normalization and semantic descriptions, origin/action guards, persistent cookie storage, browser credential behavior, bounded configuration streaming, traversal rejection, SD update policy, outbound TLS/time policy, hardware-smoke helpers, firmware size/identity enforcement, release-package checksums, exact toolchain/library pins, managed dependencies, removed-API exclusion, and the primary board's RMII clock contract. CI validates those tests and repository inputs, compiles the primary board on pushes and pull requests, enforces RAM/flash budgets and embedded version identity, builds SPIFFS, creates checksummed/versioned packages, and reuses the same build for releases. The read-only hardware smoke tool covers verified HTTPS/WSS authentication, trusted device time, REST/configuration stability, secret redaction, version/uptime, heap retention, and the browser-equivalent two-session workload. Panel actions, trust/time failure injection, certificate renewal, Wi-Fi failover, rollback, and sustained/failure-injected ESP-IDF 6 testing remain incomplete.
...
- Confirmed `version.txt` is the firmware identity source and advanced it to `AD2IOT-1125`; CMake observes the file so incremental builds cannot retain the prior identity.
...
- Installed `AD2IOT-1115` through `AD2IOT-1121` through the validated SD updater on the Ethernet device. USB serial later confirmed that the apparent `AD2IOT-1122` OTA failure had installed the image, but startup repeatedly aborted because the ESP-IDF 6 default changed the required RMII clock from GPIO 17 output to GPIO 0 input. `AD2IOT-1123` was flashed directly over USB, then validated with Ethernet link/DHCP/SNTP, public-certificate HTTPS, WSS, REST/configuration traffic, heap telemetry, and network-CLI logs. No alarm-panel control or emergency action was sent. Wi-Fi failover, certificate renewal, invalid/expired outbound trust, NTP denial, SD failure injection, abrupt-power recovery, rollback, and sustained ESP-IDF 6 workloads remain untested.
...
- `CONFIG_BOOTLOADER_APP_ROLLBACK_ENABLE` is disabled in the generated board configuration;
...
- `tools/ci/tests` now has fifty checks covering shipped service defaults, Web UI authorization structure, browser/session/action/path protections, WSS history allocation, bounded configuration streaming, SD update policy/order/UI behavior, outbound TLS/time policy, incremental version dependencies, hardware-smoke helpers, firmware size/identity enforcement, managed dependencies, removed APIs, the primary-board RMII clock contract, and release-package contents/checksums. Functional parser/redaction unit tests, broader live negative authorization tests, update failure paths, and automated hardware scheduling remain.
- Excluding vendored `{fmt}` and deferred SmartThings code, active source contains 68 `TODO`/`FIXME`/`WIP` markers and five `#if 0` blocks.
- Project-owned GPIO-mask shifts use 64-bit operands, deprecated C++ trimming adapters are gone, and the clean GCC 15 build is free of project-source warnings. The existing backlog/style debt still prevents enabling warnings-as-errors or a repository-wide format gate without a dedicated cleanup.
...
- PlatformIO Core is pinned to 6.1.19 and Python to 3.12. The earlier `pkg_resources`/setuptools compatibility shim is gone. `IntelHex` 2.3.0 is pinned and import-checked because PlatformIO's esptool 4.11 package can otherwise omit that runtime on a clean runner. PlatformIO caches are versioned by the platform, project manifest, component manifest, and dependency lock, and no longer restore broadly across incompatible package sets. GitHub Actions use explicit release versions rather than floating old majors.
...
| PlatformIO Espressif32 | 7.0.0 | 7.0.0 (ESP-IDF 6.0.0) | Upgraded and pinned to the current major platform release. |
| ESP-IDF | 6.0.0 | 6.0.x current major | Upgraded through the supported PlatformIO platform; build, boot, Ethernet, HTTPS/WSS, configuration, and diagnostic behavior are validated on the primary hardware. |
| ESP-MQTT managed component | 1.0.0 | 1.0.0 | Moved from the framework to an exact registry pin and dependency lock as required by ESP-IDF 6. |
| cJSON managed component | 1.7.19~2 | 1.7.19~2 | Moved from the framework to an exact registry pin and dependency lock as required by ESP-IDF 6. |
...
`AD2IOT-1122` completed the major migration on PlatformIO Espressif32 7.0.0, ESP-IDF 6.0.0, GCC 15.2, and Picolibc. Hardware exposed one configuration regression: IDF 6 removed the framework RMII clock Kconfig controls, so the new default supplied clock input on GPIO 0 instead of the OLIMEX board's required clock output on GPIO 17. `AD2IOT-1123` owns that choice in project Kconfig, programs the new EMAC API explicitly, and avoids a reboot loop if driver installation fails. ESP-MQTT and cJSON remain exact managed-component dependencies with a committed lock.

### API migration findings

- ESP-IDF 6 removes the dedicated LAN87xx constructor used by the board. The primary Ethernet target now uses the generic IEEE 802.3 PHY constructor and explicitly retains the former LAN87xx reset timing.
...
- Millisecond delays and UART read timeouts use `pdMS_TO_TICKS`; elapsed tick reporting uses `pdTICKS_TO_MS`. No project-owned `portTICK_PERIOD_MS` arithmetic remains.
- ESP-IDF 6 removed the public Mbed TLS SHA-256 context functions. OTA and SD image validation now use checked PSA Crypto streaming/one-shot operations; the migration also corrected an OTA helper that previously returned failure after a successful digest.
- HTTP clients handle the complete ESP-IDF 6 event enumeration, including status-code and headers-complete events. Picolibc's stricter header visibility is handled with explicit POSIX `sys/stat.h` includes rather than relying on transitive Newlib declarations.
- `esp_http_client`, the managed ESP-MQTT 1.0 nested configuration layout, `esp_netif_sntp`, `esp_vfs_fat_sdmmc_mount`, `esp_wifi_set_ps`, and `esp_http_client_set_header` remain current in the selected framework and produced no project-source deprecation diagnostics.
...
- Keep project-owned code warning-clean under GCC 15 before enabling warnings-as-errors globally; deferred SmartThings and third-party source still need their own policy.
...
- Measure Picolibc and managed-component behavior on hardware, especially HTTPS/WSS concurrency, MQTT reconnects, Ethernet link recovery, SD firmware hashing, and update/restart teardown.
...
5. Extend the completed ESP-IDF 6 hardware gate with long-duration soak, certificate renewal, invalid trust/time, Wi-Fi failover, SD write/update failure, and abrupt-power tests.

## Validation Results

This section records repository/build checks for `AD2IOT-1125`, the live activity finding from `AD2IOT-1124`, and the complete direct-USB/linked-network hardware validation for `AD2IOT-1123`.

- `tools/ci/validate_project.py`: pass.
- `python -m unittest discover -s tools/ci/tests -p "test_*.py"`: pass; fifty-five security-default, Web authorization/session, WSS history allocation, activity consolidation/fixed-width normalization/description, bounded-config, SD version-policy/order/UI, authenticated outbound TLS/time, incremental-version dependency, hardware-helper, firmware size/identity, bounded-copy, factory-reset, packaging, exact toolchain/dependency, removed-API, and RMII clock regression tests.
- `node --check contrib/webUI/flash-drive/www/activity.js` and `app.js`: pass.
- `python -m compileall -q tools/ci tools/hil`: pass.
- `git diff --check`: pass.
- Clean `pio run -e esp32-poe-iso`: pass on PlatformIO Espressif32 7.0.0, ESP-IDF 6.0.0, GCC 15.2, CMake 3.30.2, and esptool 4.11 without project-source compiler warnings or deprecated/obsolete diagnostics. The `AD2IOT-1125` build used 63,448 bytes static RAM (19.4%) and 1,581,953 bytes application flash (86.2%), below the 98,304/1,650,000-byte CI budgets and leaving 68,047 bytes of the conservative application-flash budget.
- `pio run -e esp32-poe-iso -t buildfs`: pass.
- Firmware image inspection: pass; the 1,582,368-byte `firmware.bin` contains `AD2IOT-1125` and the `Aug 16 2026 16:26:16` build timestamp.
- `tools/ci/package_release.py` smoke test: pass; 42 required release inputs, including the summarized activity module in the SD-card Web bundle, and the checksum manifest verified.
- `AD2IOT-1124` live activity inspection: authenticated trusted HTTPS returned 64 history items, and all 64 Alpha values contained fixed-width trailing padding. Running the corrected `AD2IOT-1125` JavaScript against that payload in memory produced 21 meaningful events, combining 20 related bursts with up to four updates each; no device state was changed.
...
- Latest GitHub-hosted recovery execution: commit `df47a42` passed all 50 host tests, the clean AD2IOT-1123 firmware/SPIFFS builds, embedded identity and resource budgets, 41-file packaging/checksums, and upload of the `AD2IOT-1123-Release-Package` artifact in [workflow run 31619112999](https://github.com/delphimon/AlarmDecoder-IoT/actions/runs/31619112999).
...
- ESP-IDF 6 hardware diagnosis: USB serial showed installed `AD2IOT-1122` repeatedly failing `emac_esp32_init` with `ESP_ERR_TIMEOUT` and writing a coredump. The failure occurred after the valid OTA image booted, so it was a post-install Ethernet regression rather than an OTA write failure.
- ESP-IDF 6 direct recovery: PlatformIO/esptool wrote and verified `AD2IOT-1123` on COM3 without erasing NVS or SPIFFS. Serial confirmed the expected version/build, mounted SPIFFS and SD, successful EMAC attachment and driver start, all configured service tasks, 107,368–107,496 bytes free heap with a 103,224-byte minimum, more than two minutes of uptime, and no new panic/reboot. `versionusd` reported `AD2IOT-1123` and no SD firmware image.
- ESP-IDF 6 linked-network gate: after reconnecting Ethernet and restarting, serial reported link up, DHCP `192.168.1.91`, and zero-skew SNTP synchronization. The hostname-trusted Let's Encrypt HTTPS/WSS smoke passed all active/SPIFFS/SD reads, a 1,232.6 ms WSS handshake with JSON/pong, nine configuration requests (1,936.2 ms median/1,978.2 ms p95), and 25 smaller APIs (59.3 ms median/66.5 ms p95). Free heap rose from 73,456 to 75,028 bytes after cleanup; minimum free heap was 46,664 bytes.
- ESP-IDF 6 diagnostics: authenticated network CLI reported `AD2IOT-1123`, returned 29 reboot-scoped log entries (1,441 bytes), and contained no panic, watchdog, OOM, reset-timeout, assertion, or TLS failure signature. Serial likewise remained clean. The first HTTPS private-key read logged a transient failure, but the existing bounded retry loaded the same SD material ten seconds later; hostname/trust validation and repeated TLS sessions then passed.

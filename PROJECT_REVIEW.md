...
Reviewed baseline: `AD2IOT-1124`, including a clean ESP-IDF 6.0/GCC 15 build and a summary-first activity feed with executable browser-logic tests. `AD2IOT-1123` remains the latest version validated on the primary OLIMEX hardware, including direct USB recovery, Ethernet link/DHCP/SNTP, trusted HTTPS/WSS, configuration/API workloads, and serial/network-CLI diagnostics.
...
- Confirmed `version.txt` is the firmware identity source and advanced it to `AD2IOT-1124`; CMake observes the file so incremental builds cannot retain the prior identity.
...
This section records repository/build checks for `AD2IOT-1124` and the complete direct-USB/linked-network hardware validation for `AD2IOT-1123`.

- `tools/ci/validate_project.py`: pass.
- `python -m unittest discover -s tools/ci/tests -p "test_*.py"`: pass; fifty-four security-default, Web authorization/session, WSS history allocation, activity consolidation/description, bounded-config, SD version-policy/order/UI, authenticated outbound TLS/time, incremental-version dependency, hardware-helper, firmware size/identity, bounded-copy, factory-reset, packaging, exact toolchain/dependency, removed-API, and RMII clock regression tests.
- `node --check contrib/webUI/flash-drive/www/activity.js` and `app.js`: pass.
- `python -m compileall -q tools/ci tools/hil`: pass.
- `git diff --check`: pass.
- Clean `pio run -e esp32-poe-iso`: pass on PlatformIO Espressif32 7.0.0, ESP-IDF 6.0.0, GCC 15.2, CMake 3.30.2, and esptool 4.11 without project-source compiler warnings or deprecated/obsolete diagnostics. The `AD2IOT-1124` build used 63,440 bytes static RAM (19.4%) and 1,582,973 bytes application flash (86.3%), below the 98,304/1,650,000-byte CI budgets and leaving 67,027 bytes of the conservative application-flash budget.
- `pio run -e esp32-poe-iso -t buildfs`: pass.
- Firmware image inspection: pass; the 1,583,392-byte `firmware.bin` contains `AD2IOT-1124` and the `Aug 13 2026 19:59:48` build timestamp.

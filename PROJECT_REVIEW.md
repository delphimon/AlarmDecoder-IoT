...
Reviewed baseline: `AD2IOT-1123`, including a clean ESP-IDF 6.0/GCC 15 build and direct USB hardware recovery. The image boots stably and initializes EMAC after restoring the OLIMEX board's RMII clock output; linked Ethernet and network-service validation remain pending.
...
| ESP-IDF | 6.0.0 | 6.0.x current major | Upgraded through the supported PlatformIO platform; build and boot behavior are validated, while linked network workloads remain pending. |
...
5. Install the completed ESP-IDF 6 migration on hardware and add its runtime size, heap, Ethernet, MQTT, HTTPS/WSS, SD update, and restart results to the release gate.
...
- Latest GitHub-hosted migration execution: commit `11523af` passed all 49 host tests, the clean AD2IOT-1122 firmware/SPIFFS builds, embedded identity and resource budgets, 41-file packaging/checksums, and upload of the `AD2IOT-1122-Release-Package` artifact in [workflow run 31578080984](https://github.com/delphimon/AlarmDecoder-IoT/actions/runs/31578080984).
...
- Remaining ESP-IDF 6 hardware gate: the interface did not report link or an IP address, and the configured LAN HTTPS/FTP endpoint remained unreachable. MQTT consequently logged expected DNS failures. Reconnect Ethernet before running HTTPS/WSS, trusted time, configuration, network-CLI, MQTT, update, and sustained heap tests.

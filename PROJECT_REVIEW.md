...
Reviewed baseline: `AD2IOT-1122`, including a clean ESP-IDF 6.0/GCC 15 repository build and host validation. The installed `AD2IOT-1121` hardware baseline remains validated; installing `AD2IOT-1122` is pending because the device network endpoint became unavailable before any firmware transfer or upgrade command.
...
The dependency-free host regression suite has grown from six to forty-nine checks covering externally reachable service defaults, Web UI authorization guard placement, WSS session state/history allocation, origin/action guards, persistent cookie storage, browser credential behavior, bounded configuration streaming, traversal rejection, SD update policy, outbound TLS/time policy, hardware-smoke helpers, firmware size/identity enforcement, release-package checksums, exact toolchain/library pins, managed dependencies, and removed-API exclusion. CI validates those tests and repository inputs, compiles the primary board on pushes and pull requests, enforces RAM/flash budgets and embedded version identity, builds SPIFFS, creates checksummed/versioned packages, and reuses the same build for releases. The read-only hardware smoke tool covers verified HTTPS/WSS authentication, trusted device time, REST/configuration stability, secret redaction, version/uptime, heap retention, and the browser-equivalent two-session workload. Panel actions, trust/time failure injection, certificate renewal, Wi-Fi failover, rollback, and sustained ESP-IDF 6 hardware testing remain incomplete.
...
- Confirmed `version.txt` is the firmware identity source and advanced it to `AD2IOT-1122`; CMake observes the file so incremental builds cannot retain the prior identity.
...
- Installed `AD2IOT-1115` through `AD2IOT-1121` through the validated SD updater on the Ethernet device. The final read-only `AD2IOT-1121` baseline again exercised public-certificate HTTPS, trusted time, authenticated WSS, all three configuration sources, repeated REST traffic, and heap telemetry. The device then became unreachable before the `AD2IOT-1122` FTP transfer began, so no upgrade was attempted. No alarm-panel control or emergency action was sent. Wi-Fi failover, certificate renewal, invalid/expired outbound trust, NTP denial, SD failure injection, abrupt-power recovery, rollback, and the ESP-IDF 6 image on hardware were not exercised.
...
| ESP-IDF | 6.0.0 | 6.0.x current major | Upgraded through the supported PlatformIO platform; generated configuration and clean-build behavior are validated, while hardware validation remains pending. |
...
`AD2IOT-1122` completes the next major migration on PlatformIO Espressif32 7.0.0, ESP-IDF 6.0.0, GCC 15.2, and Picolibc. ESP-MQTT and cJSON are now exact managed-component dependencies with a committed lock. The clean build reduces application flash by 18,568 bytes and adds 320 bytes of static RAM relative to `AD2IOT-1121`; hardware runtime validation is still required.
...
This section records the final repository/build checks for `AD2IOT-1122` and the latest reachable hardware baseline on `AD2IOT-1121`.

- `tools/ci/validate_project.py`: pass.
- `python -m unittest discover -s tools/ci/tests -p "test_*.py"`: pass; forty-nine security-default, Web authorization/session, WSS history allocation, bounded-config, SD version-policy/order/UI, authenticated outbound TLS/time, incremental-version dependency, hardware-helper, firmware size/identity, bounded-copy, factory-reset, packaging, exact toolchain/dependency, and removed-API regression tests.
- `node --check contrib/webUI/flash-drive/www/app.js`: pass.
- `python -m compileall -q tools/ci tools/hil`: pass.
- `git diff --check`: pass.
- Clean `pio run -e esp32-poe-iso`: pass on PlatformIO Espressif32 7.0.0, ESP-IDF 6.0.0, GCC 15.2, CMake 3.30.2, and esptool 4.11 without project-source compiler warnings or deprecated/obsolete diagnostics. The build used 63,440 bytes static RAM (19.4%) and 1,582,905 bytes application flash (86.3%), below the 98,304/1,650,000-byte CI budgets. Relative to `AD2IOT-1121`, this adds 320 bytes RAM and saves 18,568 bytes linked flash, leaving 67,095 bytes of the conservative application-flash budget.
- `pio run -e esp32-poe-iso -t buildfs`: pass.
- Firmware image inspection: pass; the 1,583,312-byte `firmware.bin` contains `AD2IOT-1122` and the `Aug 12 2026 00:47:17` build timestamp.
...
- Latest completed GitHub-hosted execution before this migration: commit `848e422` passed all 47 host tests, the clean AD2IOT-1121 firmware/SPIFFS builds, embedded identity and resource budgets, 41-file packaging/checksums, and upload of the `AD2IOT-1121-Release-Package` artifact in [workflow run 31569058870](https://github.com/delphimon/AlarmDecoder-IoT/actions/runs/31569058870). The `AD2IOT-1122` workflow result is pending this branch push.
...
- ESP-IDF 6 hardware gate: pending. Before staging, a read-only `AD2IOT-1121` smoke run passed all active/SPIFFS/SD reads, trusted time, WSS JSON/pong, and 13 REST requests with no retained heap loss (49,444 to 49,052 bytes; 17,920-byte minimum). The device then stopped responding on its LAN-resolved HTTPS/FTP address; no `AD2IOT-1122` file transfer or `upgradeusd` command occurred, and no serial adapter beyond the host's COM1 was available.

...
- [x] CI/TEST: expand the host suite to 50 checks with a primary-board RMII clock regression guard; pass the clean GitHub firmware/SPIFFS build, identity/size gates, package checksums, and `AD2IOT-1123-Release-Package` upload.
...
- [x] TEST/HARDWARE: restore Ethernet and verify link/DHCP/SNTP, hostname-trusted Let's Encrypt HTTPS, authenticated WSS, active/SPIFFS/SD configuration, 34 REST requests, network-CLI version/log access, a 46,664-byte heap low-water mark, no retained heap loss, and no critical serial/network-CLI signature on `AD2IOT-1123`.
...
- [x] CI: repair clean ESP-IDF 5.5 runner builds by pinning the `IntelHex` runtime required by esptool 4.11, preflighting its import, and isolating PlatformIO caches by complete toolchain input without broad stale-package restores.
